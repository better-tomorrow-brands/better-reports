...
/**
 * GET /api/reports/facebook-adsets
 *   ?from=&to=&campaignId=                           → aggregated rows per ad set
 *   ?from=&to=&adsetId=&groupBy=day                  → daily time-series for a single ad set
 */
export async function GET(request: Request) {
  try {
    ...
    // ── Daily time-series mode ──────────────────────────────────────────────────
    if (groupBy === "day") {
      const adsetId = url.searchParams.get("adsetId");
      const adset = url.searchParams.get("adset");
      if (!from || !to || (!adsetId && !adset)) {
        return NextResponse.json({ error: "from, to, and adsetId (or adset) required for groupBy=day" }, { status: 400 });
      }
      ...
      return NextResponse.json({ adsetId, from, to, rows: result });
    }
